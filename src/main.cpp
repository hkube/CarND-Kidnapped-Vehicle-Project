...
int main()
{
  ...
  const ParticleFilter::Sigma3 sigma_pos = {0.3, 0.3, 0.01}; // GPS measurement uncertainty [x [m], y [m], theta [rad]]
  const ParticleFilter::Sigma2 sigma_landmark = {0.3, 0.3}; // Landmark measurement uncertainty [x [m], y [m]]
    ...
    if (length && length > 2 && data[0] == '4' && data[1] == '2')
    {
      ...
      if (s != "") {
        ...
        if (event == "telemetry") {
          ...
          vector<ParticleFilter::Particle> particles = pf.getParticles();
